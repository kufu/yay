...
問題は、このfooメソッドがどのようなシグネチャに依ります。次のように宣言されている場合は、呼び出し時にキーワード引数を渡すことで、キーワード引数を暗黙的にハッシュに変換する処理が入ります。
...
この機能も、キワード引数の未定義引数として `**kwrest` という引数で受け取ることができますが、未だに多くのライブラリなどで使われているイディオムなので、覚えておく価値はあります。
